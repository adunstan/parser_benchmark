...
DROP SCHEMA IF EXISTS parser_test CASCADE;

CREATE SCHEMA parser_test;
SET search_path TO parser_test;
...
CREATE FUNCTION _pg_index_position(oid, smallint) RETURNS int
    ...
    AS $$
SELECT (ss.a).n FROM
  (SELECT parser_test._pg_expandarray(indkey) AS a
...
$$SELECT
  CASE WHEN $1 IN (25, 1042, 1043) /* text, char, varchar */
       THEN CASE WHEN $2 = -1 /* default typmod */
                 ...
                 ELSE parser_test._pg_char_max_length($1, $2) *
...
CREATE VIEW triggered_update_columns AS
    ...
    FROM pg_namespace n, pg_class c, pg_trigger t,
         (SELECT tgoid, (ta0.tgat).x AS tgattnum, (ta0.tgat).n AS tgattpos
          FROM (SELECT oid AS tgoid, parser_test._pg_expandarray(tgattr) AS tgat FROM pg_trigger) AS ta0) AS ta,
         pg_attribute a
